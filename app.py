...
# --- МАТЕМАТИЧЕСКИЕ ФУНКЦИИ ДЛЯ ТОЧНОГО УЧЕТА ---
def calc_active_value(row):
    if row['Тип оплаты'] == 'Абонемент':
        return row['Сумма'] if not row['Должник'] else 0
    else:
        return row['Баланс занятий'] * PRICE_PER_SESSION if row['Баланс занятий'] > 0 else 0

def calc_debt_value(row):
    if row['Тип оплаты'] == 'Абонемент':
        return row['Сумма'] if row['Должник'] else 0
    else:
...
if page == "📊 Дашборд":
    ...
    if not df.empty:
        ...
        df['Активная ценность'] = df.apply(calc_active_value, axis=1)
        df['Сумма долга'] = df.apply(calc_debt_value, axis=1)
        ...
        with c1:
            ...
            st.metric("Ценность активной базы", f"{int(active_subscriptions_value)} ₽", delta=delta_val, delta_color="normal", help="Сумма всех оплаченных абонементов и остатка купленных разовых тренировок.")
        with c2:
        ...
        # Замена пустых дат на красивый текст, чтобы не было "NaT"
        view_df['Оплачено до'] = view_df['Оплачено до'].fillna('Нет')
        ...
        def style_rows(row):
            # Проверяем долг. Так как в view_df 'Оплачено до' может быть строкой 'Нет', обрабатываем безопасно
            is_debt = False
            if row['Тип оплаты'] == 'Абонемент':
                is_debt = row['Оплачено до'] == 'Нет' or pd.to_datetime(row['Оплачено до'], errors='coerce') < today_dt
            else:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        if st.form_submit_button("СОХРАНИТЬ"):
            if not name:
                ...
                save_data(save_df)
                st.success(f"Сохранено! Не забудь скачать бэкап.")
                st.rerun()
