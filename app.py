...
if page == "📊 Дашборд":
    ...
    if not df.empty:
        # --- ФИНАНСОВЫЕ РАСЧЕТЫ ---
        # ИСПРАВЛЕНО: Защита от пустых значений (NaT), которые вызывали ошибку TypeError
        valid_dates = df.dropna(subset=['Оплачено до']) 
        
        # Фильтруем тех, кто оплатил (дата оплаты >= сегодня)
        paid_students = len(valid_dates[valid_dates['Оплачено до'] >= date.today()])
        
        # ИСПРАВЛЕНО: Синхронизируем логику подсчета долга с сайдбаром (строго дата < сегодня)
        unpaid_students = len(valid_dates[valid_dates['Оплачено до'] < date.today()])
